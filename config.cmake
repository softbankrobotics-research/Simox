...
function(add_visualization_qt4)
	MESSAGE (STATUS "Using Qt4: " ${QT_INCLUDE_DIR})
	...
	set(Simox_VISUALIZATION_LIBS "${Simox_VISUALIZATION_LIBS}" PARENT_SCOPE)
	set(Simox_VISUALIZATION_INCLUDE_PATHS "${Simox_VISUALIZATION_INCLUDE_PATHS}" PARENT_SCOPE)
	set(Simox_EXTERNAL_CMAKE_INCLUDE "${Simox_EXTERNAL_CMAKE_INCLUDE}" PARENT_SCOPE)
endfunction()


    
function(add_visualization_qt5)
	...
	set(Simox_VISUALIZATION_LIBS "${Simox_VISUALIZATION_LIBS}" PARENT_SCOPE)
	set(Simox_VISUALIZATION_INCLUDE_PATHS "${Simox_VISUALIZATION_INCLUDE_PATHS}" PARENT_SCOPE)
	set(Simox_VISUALIZATION_COMPILE_FLAGS "${Simox_VISUALIZATION_COMPILE_FLAGS}" PARENT_SCOPE)
endfunction()
...
IF (NOT Simox_CONFIGURED)
    ...
    IF(UNIX)
        ...
        #MESSAGE ("Simox_INSTALL_LIB_DIR: ${CMAKE_INSTALL_PREFIX}/${Simox_INSTALL_LIB_DIR}")
        SET(CMAKE_INSTALL_RPATH "${CMAKE_INSTALL_PREFIX}/${Simox_INSTALL_LIB_DIR}")
    ...
    OPTION(Simox_USE_OGRE_VISUALIZATION "Use Ogre for visualization" OFF)
    ...
    elseif (Simox_USE_OGRE_VISUALIZATION)
		MESSAGE(STATUS "Searching Ogre and Qt...")
		MESSAGE (STATUS "Appending ogre search path /usr/share/OGRE/cmake/modules/")
		LIST (APPEND CMAKE_MODULE_PATH "/usr/share/OGRE/cmake/modules/")
        FIND_PACKAGE(OGRE)
        if (OGRE_FOUND)
            MESSAGE (STATUS "Found Ogre:" ${OGRE_INCLUDE_DIRS})
            ##INCLUDE_DIRECTORIES(${OPENSCENEGRAPH_INCLUDE_DIRS})
        endif()
		if ( QT_FOUND )
			add_visualization_qt4()
        ...
        FIND_PACKAGE(ASSIMP)
        if (NOT ASSIMP_FOUND)
            MESSAGE ("Warning: assimp lib not find via cmake, usign std settings...")
        ...
        if ( (QT_FOUND OR Qt5_FOUND) AND OGRE_FOUND)# AND ASSIMP_FOUND)
            ...
            if (UNIX)
                ...
                FIND_PACKAGE(OpenGL)
                MESSAGE (STATUS "Adding OpenGL libraries:" ${OPENGL_LIBRARIES})
                LIST (APPEND Simox_VISUALIZATION_LIBS ${OPENGL_LIBRARIES})
                MESSAGE (STATUS "Searching X11 libraries")
                FIND_PACKAGE(X11)
                MESSAGE (STATUS "Adding X11 libraries:" ${X11_LIBRARIES})
                LIST (APPEND Simox_VISUALIZATION_LIBS ${X11_LIBRARIES})
            endif()
