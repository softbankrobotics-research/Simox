...
IF (NOT Simox_CONFIGURED)
    ...
    if (Simox_VISUALIZATION_TYPE STREQUAL "Qt3D")
        if(Simox_USE_QT4)
            MESSAGE("Config forces Qt4, but Qt4 does not support Qt3D. At least Qt5 version >= 5.9.0 is needed!")
        endif()
        FIND_PACKAGE(Qt5 5.9.0 QUIET COMPONENTS OpenGL Core Gui Widgets 3DCore 3DExtras 3DRender 3DInput)
        if(NOT Qt5_FOUND)
            MESSAGE("Could not load QT5 for Qt3D")
        else ()
    ...
    #### VISUALIZATION
    ##################
    SET (Simox_VISUALIZATION_TYPE "Coin3D" CACHE STRING "Possible options: None, Coin3D, Qt3D")
    SET_PROPERTY(CACHE Simox_VISUALIZATION_TYPE PROPERTY STRINGS "None" "Coin3D" "Qt3D")
    ...
    elseif (Simox_VISUALIZATION_TYPE STREQUAL "Qt3D")
        if(NOT Qt5_FOUND)
            MESSAGE (STATUS "Did not find Qt5 version >= 5.9.0. Disabling Qt3D.")
        else()
            MESSAGE(STATUS "Using Qt3D for visualization.")
            SET (Simox_VISUALIZATION TRUE)
            SET(Simox_BUILD_EXAMPLES ON)
            SET (Simox_VISUALIZATION_COMPILE_FLAGS "${Simox_VISUALIZATION_COMPILE_FLAGS} -DSimox_VISUALIZATION -DSimox_USE_QT3D_VISUALIZATION")
            MESSAGE (STATUS "Found Qt5: " ${Qt5Gui_INCLUDE_DIRS}  ${Qt5OpenGL_INCLUDE_DIRS} ${Qt5Gui_LIBRARIES})
            MESSAGE (STATUS "Qt5 compiler flags: " ${Qt5Gui_EXECUTABLE_COMPILE_FLAGS} " " ${Qt5OpenGL_EXECUTABLE_COMPILE_FLAGS})
            LIST (APPEND Simox_EXTERNAL_LIBRARY_CMAKE_INCLUDE ${Qt5Gui_INCLUDE_DIRS}  ${Qt5OpenGL_INCLUDE_DIRS})
            LIST (APPEND Simox_EXTERNAL_LIBRARIES ${Qt5Gui_LIBRARIES} ${Qt5OpenGL_LIBRARIES} Qt5::Core Qt5::Gui Qt5::Widgets Qt5::3DCore Qt5::3DExtras Qt5::3DRender Qt5::3DInput)
            #QT5
            LIST (APPEND Simox_VISUALIZATION_LIBS ${Qt5Gui_LIBRARIES} ${Qt5OpenGL_LIBRARIES})
            LIST (APPEND Simox_VISUALIZATION_INCLUDE_PATHS ${Qt5Gui_INCLUDE_DIRS}  ${Qt5OpenGL_INCLUDE_DIRS} )
            if (UNIX)
