...
namespace VirtualRobot
{
    ...
    void SphereApproximator::buildVertexFaceMapping(SphereApproximation& storeResult)
    {
        ...
        {
			TriMeshModel::TriangleFace f = storeResult.faces[i];
            FaceIndex fi;
    ...
    void SphereApproximator::generateIcosahedron(GraphData& gd)
    {
		...
		TriMeshModel::TriangleFace f[20];
        f[0].set(5,  8,  9);
        f[1].set(5, 10,  8);
        f[2].set(6, 12,  7);
        f[3].set(6,  7, 11);
    ...
    {
        // buffer new faces
        vector<TriMeshModel::TriangleFace> newFaces;
        ...
        for (int f = 0 ; f < int(gd.faces.size()) ; f++)
        {
			...
			TriMeshModel::TriangleFace fa[4];
