...
namespace VirtualRobot
{
     ...
     */
    CoinVisualization::CoinVisualization(SoNode* visualizationNode, float margin) :
     ...
     */
    TriMeshModelPtr CoinVisualization::getTriMeshModel()
    {
        if (!triMeshModel)
        {
            createTriMeshModel();
     ...
     */
    void CoinVisualization::InventorTriangleCB(void* data, SoCallbackAction* action,
    ...
    void CoinVisualization::setGlobalPose(const Eigen::Matrix4f& m)
    {
        ...
        {
            SbMatrix m(reinterpret_cast<SbMat*>(localPose.data()));
            // mm -> m
            m[3][0] *= 0.001f;
            m[3][1] *= 0.001f;
            m[3][2] *= 0.001f;
    ...
    void CoinVisualization::attachVisualization(const std::string& name, VisualizationPtr v)
    {
        Visualization::attachVisualization(name, v);

        std::shared_ptr<CoinVisualization> coinVisualizationNode = std::dynamic_pointer_cast<CoinVisualization>(v);
    ...
    void CoinVisualization::detachVisualization(const std::string& name)
    {
        Visualization::detachVisualization(name);
    ...
    VirtualRobot::VisualizationPtr CoinVisualization::clone(bool deepCopy, float scaling)
    {
        ...
        CoinVisualizationNodePtr p(new CoinVisualization(newModel));
        ...
        std::map< std::string, VisualizationPtr >::const_iterator i = attachedVisualizations.begin();

        while (i != attachedVisualizations.end())
        {
            VisualizationPtr attachedClone = i->second->clone(deepCopy, scaling);
    ...
    std::string CoinVisualization::getType()
    {
        return CoinVisualizationFactory::getName();
    }

    void CoinVisualization::setupVisualization(bool showVisualization, bool showAttachedVisualizations)
    {
        Visualization::setupVisualization(showVisualization, showAttachedVisualizations);
    ...
    void CoinVisualization::scale(Eigen::Vector3f& scaleFactor)
    {
