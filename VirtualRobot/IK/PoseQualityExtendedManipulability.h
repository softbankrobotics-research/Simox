...
namespace VirtualRobot
{
    ...
    */
    ...
    public:
        ...
        */
        PoseQualityExtendedManipulability(VirtualRobot::JointSetPtr rns, PoseQualityManipulability::ManipulabilityIndexType i = PoseQualityManipulability::eMinMaxRatio);
        ~PoseQualityExtendedManipulability();

        virtual float getPoseQuality() override;
        ...
        */
        virtual float getPoseQuality(const Eigen::VectorXf& direction) override;
        virtual float getManipulability(const Eigen::VectorXf& direction, int considerFirstSV = -1) override;
        ...
        */
        ...
        virtual bool consideringJointLimits() override;

        virtual PoseQualityMeasurementPtr clone(RobotPtr newRobot) override;

        void getSelfDistParameters(float &storeAlpha, float &storeBeta);

    protected:

        bool getDetailedAnalysis(DifferentialIKPtr jacobian, JointSetPtr rns, extManipData& storeData, int considerFirstSV = 0);
        bool getDetailedAnalysis(DifferentialIKPtr jacobian, JointSetPtr rns, extManipData& storeData, bool (&dims)[6], int considerFirstSV = 0);
        bool getDetailedAnalysis(const Eigen::MatrixXf& jac, const std::vector<ModelJointPtr>& joints, extManipData& storeData, bool (&dims)[6], int considerFirstSV = 0);

        float getPoseQuality(DifferentialIKPtr jacobian, JointSetPtr rns, PoseQualityManipulability::ManipulabilityIndexType i, int considerFirstSV);
        ...
        */
        ...
        void getPenalizations(const std::vector<ModelJointPtr>& joints, Eigen::VectorXf& penLo, Eigen::VectorXf& penHi);
        ...
        */
        ...
        void getObstaclePenalizations(JointSetPtr rns, const Eigen::Vector3f& obstVect, const Eigen::MatrixXf& jac, Eigen::MatrixXf& penObstLo, Eigen::MatrixXf& penObstHi);
        void getObstaclePenalizations(const std::vector<ModelJointPtr>& joints, const Eigen::Vector3f& obstVect, const Eigen::MatrixXf& jac, Eigen::MatrixXf& penObstLo, Eigen::MatrixXf& penObstHi);
